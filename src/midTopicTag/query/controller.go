...
// @Param update_at query []string false "update_at"
// @Param is_use query string false "is_use"
// @Router /api/topic/query-topic/tag/mid/pading [post]
func QueryTopicTagMidPadingController(c *gin.Context) {
	// 做一下判断不允许传参过多
	appG := app.Gin{C: c}

	jsonString := app.GetPostJson(c)
	jsonData := &QueryTopicTagMidPadingParams{}
	...
	result := QueryTopicTagMidPading(c, *jsonData)
	appG.Response(http.StatusOK, result.Code, result.Data)
}
